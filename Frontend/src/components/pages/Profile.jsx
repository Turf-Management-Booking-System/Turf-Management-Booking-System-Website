...
const EditProfile = () => {
  ...
  const updateProfileHandler = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(`http://localhost:4000/api/v1/auth/updateProfile/${user?._id}`, requestData, {
        headers: {
  ...
  const handleImageChangeHandler = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        `http://localhost:4000/api/v1/auth/upload-Profile-Image/${user?._id}`,
        formData,
        {
          headers: {
