...
function Home() {
  ...
  const nextTestimonials = () => {
    setIndex((prevIndex) =>
      prevIndex + itemsPerPage < testimonials.length ? prevIndex + itemsPerPage : 0
    );
  };

  const prevTestimonials = () => {
    setIndex((prevIndex) =>
      prevIndex - itemsPerPage >= 0 ? prevIndex - itemsPerPage : testimonials.length - itemsPerPage
    );
  };
        ...
        <div className="overflow-hidden">
          ...
          >
            {testimonials.slice(index, index + itemsPerPage).map((testimonial) => (
              <div key={testimonial.id} className="relative bg-white dark:bg-gray-700 p-6 rounded-lg shadow-lg flex flex-col items-center text-center">
                {/* User Image (Half Inside, Half Outside) */}
                <div className="absolute -top-8">
                  <img
                    src={testimonial.image}
                    alt={testimonial.name}
                    className="w-20 h-20 rounded-full border-4 border-gray-300 dark:border-gray-500"
                  />
                </div>
